...
extension UIViewController: ViperModuleTransitionHandler {
    ...
    public func createEmbeddableModuleUsingFactory(_ moduleFactory: ViperModuleFactory, configurationBlock: (ViperModuleInput) -> ViperModuleOutput) {
        self.createEmbeddableModuleUsingFactory(moduleFactory, configurationBlock: configurationBlock, lazzyAllocation: false)
    }

    public func createEmbeddableModuleUsingFactory(_ moduleFactory: ViperModuleFactory, configurationBlock: (ViperModuleInput) -> ViperModuleOutput, lazzyAllocation: Bool) {
        fatalError()
    }
    ...
    public func closeModulesUntil(_ transitionHandler: ViperModuleTransitionHandler?, animated: Bool, completion: ModuleCloseCompletionBlock?) {
        ...
        func skip(_ viewController: UIViewController) -> Bool {
            guard viewController.skipOnDismiss else { return false }

            if let moduleInput = self.moduleInput {
                moduleInput.moduleDidSkipOnDismiss()
            }
